...
CURSOR=RelayConnectionInterface.CURSOR;var NODE=RelayConnectionInterface.NODE;
var EMPTY='';
var FILTER_CALLS='__filterCalls__';
var FORCE_INDEX='__forceIndex__';
var RANGE='__range__';
var PATH='__path__';var 
APPEND=GraphQLMutatorConstants.APPEND;var PREPEND=GraphQLMutatorConstants.PREPEND;var REMOVE=GraphQLMutatorConstants.REMOVE;var 
...
this._storage.push(this._cachedRecords);}}RelayRecordStore.prototype.
...
storageKey):invariant(false):undefined;
...
return this._rootCallMap[storageKey][identifyingArgValue];}else 
if(this._cachedRootCallMap.hasOwnProperty(storageKey)){
return this._cachedRootCallMap[storageKey][identifyingArgValue];}};RelayRecordStore.prototype.
...
this._cacheWriter.writeRootCall(storageKey,identifyingArgValue,dataID);}};RelayRecordStore.prototype.
...
getRecordState = function getRecordState(dataID){
...
return 'EXISTENT';};RelayRecordStore.prototype.
...
if(cachedPath){
cacheWriter.writeField(dataID,'__path__',cachedPath,typeName);}}};RelayRecordStore.prototype.
...
getPathToRecord = function getPathToRecord(
dataID)
{
var path=this._getField(dataID,PATH);
return path;};RelayRecordStore.prototype.
...
!
this._queuedRecords?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayRecordStore.hasOptimisticUpdate(): Optimistic updates require ' + 
'queued records.'):invariant(false):undefined;

return this._queuedRecords.hasOwnProperty(dataID);};RelayRecordStore.prototype.
...
return record?record.__mutationIDs__:null;};RelayRecordStore.prototype.
...
hasMutationError = function hasMutationError(dataID){
if(this._queuedRecords){
var record=this._queuedRecords[dataID];
return !!(
...
return false;};RelayRecordStore.prototype.





setMutationErrorStatus = function setMutationErrorStatus(dataID,hasError){
!
this._queuedRecords?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayRecordStore.setMutationErrorStatus(): Can only set the ' + 
'mutation status of queued records.'):invariant(false):undefined;
...
record.__status__,
hasError);};RelayRecordStore.prototype.
...
if(this._cacheWriter){
this._cacheWriter.writeNode(dataID,null);}}};RelayRecordStore.prototype.




getType = function getType(dataID){

return this._getField(dataID,'__typename');};RelayRecordStore.prototype.
...
storageKey)
{
return this._getField(dataID,storageKey);};RelayRecordStore.prototype.





putField = function putField(
dataID,
storageKey,
value)
{
...
this._cacheWriter.writeField(dataID,storageKey,value,typeName);}};RelayRecordStore.prototype.
...
if(!this._queuedRecords && this._cacheWriter){
this._cacheWriter.writeField(dataID,storageKey,null);}};RelayRecordStore.prototype.
...
getLinkedRecordID = function getLinkedRecordID(
dataID,
storageKey)
{
...
typeof field === 'object' && 
field !== null && 
!Array.isArray(field))?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayRecordStore.getLinkedRecordID(): Expected field `%s` for record ' + 
'`%s` to have a linked record.',
storageKey,
dataID):invariant(false):undefined;

return field.__dataID__;};RelayRecordStore.prototype.





putLinkedRecordID = function putLinkedRecordID(
parentID,
storageKey,
recordID)
{
...
parentID,
recordID):invariant(false):undefined;
...
recordID,
parentID):invariant(false):undefined;
...
if(!this._queuedRecords && this._cacheWriter){
this._cacheWriter.writeField(parentID,storageKey,fieldValue);}};RelayRecordStore.prototype.
...
getLinkedRecordIDs = function getLinkedRecordIDs(
dataID,
storageKey)
{
var field=this._getField(dataID,storageKey);
...
!(
typeof item === 'object' && item.__dataID__)?process.env.NODE_ENV !== 'production'?invariant(false,
...
return item.__dataID__;});};RelayRecordStore.prototype.
...
putLinkedRecordIDs = function putLinkedRecordIDs(
parentID,
storageKey,
recordIDs)
{var _this=this;
...
if(!this._queuedRecords && this._cacheWriter){
this._cacheWriter.writeField(parentID,storageKey,records);}};RelayRecordStore.prototype.
...
getConnectionIDsForRecord = function getConnectionIDsForRecord(
dataID)
{
...
return null;};RelayRecordStore.prototype.
...
getConnectionIDsForField = function getConnectionIDsForField(
dataID,
schemaName)
{
...
return connectionIDs;};RelayRecordStore.prototype.





getRangeForceIndex = function getRangeForceIndex(
connectionID)
{
...
return forceIndex || 0;};RelayRecordStore.prototype.
...
getRangeFilterCalls = function getRangeFilterCalls(
connectionID)
{
return this._getField(connectionID,FILTER_CALLS);};RelayRecordStore.prototype.
...
getRangeMetadata = function getRangeMetadata(
connectionID,
calls)
{var _this2=this;
...
'`edges` has been fetched.'):undefined;}
...
null;var _range$retrieveRangeInfoForQuery=




range.retrieveRangeInfoForQuery(calls,queuedRecord);var diffCalls=_range$retrieveRangeInfoForQuery.diffCalls;var pageInfo=_range$retrieveRangeInfoForQuery.pageInfo;var requestedEdgeIDs=_range$retrieveRangeInfoForQuery.requestedEdgeIDs;
...
filter(function(edge){return _this2._getRecord(edge.nodeID);});}else 
{
requestedEdges = [];}
...
filterCalls:filterCalls,
pageInfo:pageInfo,
requestedEdges:requestedEdges};};RelayRecordStore.prototype.
...
!
!this._queuedRecords?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayRecordStore.putRange(): Cannot create a queued range.'):invariant(false):undefined;
...
cacheWriter.writeField(connectionID,RANGE,range);}};RelayRecordStore.prototype.
...
hasRange = function hasRange(connectionID){
return !!this._getField(connectionID,RANGE);};RelayRecordStore.prototype.





putRangeEdges = function putRangeEdges(
connectionID,
calls,
pageInfo,
edges)
...
var range=this._getField(connectionID,RANGE);
!
range?process.env.NODE_ENV !== 'production'?invariant(false,
...
if(!this._queuedRecords && this._cacheWriter){
this._cacheWriter.writeField(connectionID,RANGE,range);}};RelayRecordStore.prototype.
...
applyRangeUpdate = function applyRangeUpdate(
connectionID,
edgeID,
operation)
{
if(this._queuedRecords){
this._applyOptimisticRangeUpdate(connectionID,edgeID,operation);}else 
{
this._applyServerRangeUpdate(connectionID,edgeID,operation);}};RelayRecordStore.prototype.
...
removeRecord = function removeRecord(dataID){
delete this._records[dataID];
if(this._queuedRecords){
delete this._queuedRecords[dataID];}

if(this._cachedRecords){
delete this._cachedRecords[dataID];}};RelayRecordStore.prototype.
...
_getRangeEdgeData = function _getRangeEdgeData(edgeID){
var nodeID=this.getLinkedRecordID(edgeID,NODE);
!
nodeID?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayRecordStore: Expected edge `%s` to have a `node` record.',
edgeID):invariant(false):undefined;

return {
__dataID__:edgeID,
cursor:this.getField(edgeID,CURSOR),
node:{
__dataID__:nodeID}};};RelayRecordStore.prototype.




_applyOptimisticRangeUpdate = function _applyOptimisticRangeUpdate(
connectionID,
edgeID,
operation)
{
...
{
queue.push(edgeID);}};RelayRecordStore.prototype.



_applyServerRangeUpdate = function _applyServerRangeUpdate(
connectionID,
edgeID,
operation)
{
!
this._records?process.env.NODE_ENV !== 'production'?invariant(false,
...
_addConnectionForNode = function _addConnectionForNode(
connectionID,
nodeID)
{
...
connectionMap[connectionID] = true;};RelayRecordStore.prototype.





_removeConnectionForNode = function _removeConnectionForNode(
connectionID,
nodeID)
{
var connectionMap=this._nodeConnectionMap[nodeID];
...
delete this._nodeConnectionMap[nodeID];}}};RelayRecordStore.prototype.
...
_getRecord = function _getRecord(dataID){
if(this._queuedRecords && this._queuedRecords.hasOwnProperty(dataID)){
return this._queuedRecords[dataID];}else 
if(this._records.hasOwnProperty(dataID)){
return this._records[dataID];}else 
if(this._cachedRecords){
return this._cachedRecords[dataID];}};RelayRecordStore.prototype.
...
_getRecordForWrite = function _getRecordForWrite(dataID){
...
return record;};RelayRecordStore.prototype.
...
_getField = function _getField(dataID,storageKey){
...
return undefined;};RelayRecordStore.prototype.
...
_setClientMutationID = function _setClientMutationID(record){
var clientMutationID=this._clientMutationID;
!
clientMutationID?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayRecordStore: _clientMutationID cannot be null/undefined.'):invariant(false):undefined;
...
return calls.filter(function(call){return !RelayConnectionInterface.isConnectionCall(call);});}
