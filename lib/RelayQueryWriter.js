...
EDGES=RelayConnectionInterface.EDGES;var NODE=RelayConnectionInterface.NODE;var PAGE_INFO=RelayConnectionInterface.PAGE_INFO;var 
...
RelayQueryWriter=(function(_RelayQueryVisitor){_inherits(RelayQueryWriter,_RelayQueryVisitor);
...
this._updateTrackedQueries = !!(options && options.updateTrackedQueries);}RelayQueryWriter.prototype.


getRecordStore = function getRecordStore(){
return this._store;};RelayQueryWriter.prototype.


getRecordTypeName = function getRecordTypeName(
field,
recordID,
payload)
{
...
return typeName || null;};RelayQueryWriter.prototype.
...
writePayload = function writePayload(
node,
recordID,
responseData,
path)
...
this.visit(node,state);};RelayQueryWriter.prototype.
...
recordCreate = function recordCreate(
recordID)
{
this._changeTracker.createID(recordID);};RelayQueryWriter.prototype.
...
recordUpdate = function recordUpdate(
recordID)
{
this._changeTracker.updateID(recordID);};RelayQueryWriter.prototype.





hasChangeToRecord = function hasChangeToRecord(recordID){
return this._changeTracker.hasChange(recordID);};RelayQueryWriter.prototype.
...
return this._changeTracker.isNewRecord(recordID);};RelayQueryWriter.prototype.





createRecordIfMissing = function createRecordIfMissing(
node,
recordID,
typeName,
path)
...
this._queryTracker.trackNodeForID(node,recordID,path);}};RelayQueryWriter.prototype.
...
!(
responseData !== undefined)?process.env.NODE_ENV !== 'production'?invariant(false,
...
!(
typeof responseData === 'object' && responseData !== null)?process.env.NODE_ENV !== 'production'?invariant(false,
...
this.traverse(root,state);};RelayQueryWriter.prototype.
...
!(
this._store.getRecordState(recordID) === RelayRecordState.EXISTENT)?process.env.NODE_ENV !== 'production'?invariant(false,
...
if(field.isScalar()){
this._writeScalar(field,state,recordID,fieldData);}else 
if(field.isConnection()){
this._writeConnection(field,state,recordID,fieldData);}else 
if(field.isPlural()){
this._writePluralLink(field,state,recordID,fieldData);}else 
{
this._writeLink(field,state,recordID,fieldData);}};RelayQueryWriter.prototype.
...
_writeScalar = function _writeScalar(
field,
state,
recordID,
nextValue)
...
this.recordUpdate(recordID);};RelayQueryWriter.prototype.
...
_writeConnection = function _writeConnection(
field,
state,
recordID,
connectionData)
...
this._traverseConnection(field,field,connectionState);};RelayQueryWriter.prototype.
...
_traverseConnection = function _traverseConnection(
connection,
node,
state)
{var _this2=this;
...
_this2._writeEdges(connection,child,state);}else 
if(child.getSchemaName() !== PAGE_INFO){


_this2.visit(child,state);}}else 

{

_this2._traverseConnection(connection,child,state);}});};RelayQueryWriter.prototype.
...
_writeEdges = function _writeEdges(
connection,
edges,
state)
{var _this3=this;var 
...
!(
typeof connectionData === 'object' && connectionData !== null)?process.env.NODE_ENV !== 'production'?invariant(false,
...
!
Array.isArray(edgesData)?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayQueryWriter: Cannot write edges for connection `%s` on record ' + 
...
!
RelayConnectionInterface.hasRangeCalls(rangeCalls)?process.env.NODE_ENV !== 'production'?invariant(false,
'RelayQueryWriter: Cannot write edges for connection `%s` on record ' + 
...
!
rangeInfo?process.env.NODE_ENV !== 'production'?invariant(false,
...
!(
typeof edgeData === 'object' && edgeData)?process.env.NODE_ENV !== 'production'?invariant(false,
...
!(
typeof nodeData === 'object')?process.env.NODE_ENV !== 'production'?invariant(false,
...
prevEdge && _this3._store.getLinkedRecordID(prevEdge.edgeID,NODE) || 
generateClientID();
...
if(isUpdate){
this.recordUpdate(connectionID);}};RelayQueryWriter.prototype.
...
_writePluralLink = function _writePluralLink(
field,
state,
recordID,
fieldData)
...
var storageKey=field.getStorageKey();
!
Array.isArray(fieldData)?process.env.NODE_ENV !== 'production'?invariant(false,
...
!(
typeof nextRecord === 'object' && nextRecord)?process.env.NODE_ENV !== 'production'?invariant(false,
...
_this4.isNewRecord(nextLinkedID);
...
_writeLink = function _writeLink(
field,
state,
recordID,
fieldData)
...
var storageKey=field.getStorageKey();
!(
typeof fieldData === 'object' && fieldData !== null)?process.env.NODE_ENV !== 'production'?invariant(false,
...
nodeID:null,
recordID:nextLinkedID,
responseData:fieldData});};return RelayQueryWriter;})(RelayQueryVisitor);
