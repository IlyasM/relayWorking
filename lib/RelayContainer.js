...
route=context.route;
!(
route && typeof route.name === 'string')?process.env.NODE_ENV !== 'production'?invariant(false,
...
this.state = {
variables:{},
queryData:{}};}RelayContainer.prototype.
...
callback)
{
this._runVariables(partialVariables,callback,false);};RelayContainer.prototype.
...
callback)
{
this._runVariables(partialVariables,callback,true);};RelayContainer.prototype.
...
!
Array.isArray(queryData)?process.env.NODE_ENV !== 'production'?invariant(false,
...
if(dataIDs.length){
fragmentPointer = new GraphQLFragmentPointer(dataIDs,fragment);}}else 
...
if(forceFetch || !shallowEqual(nextVariables,lastVariables)){var _createQuerySetAndFragmentPointers2=
...
var onReadyStateChange=ErrorUtils.guard(function(readyState){var 
...
if(callback){
callback.call(_this2.refs.component,_extends({},readyState,{mounted:mounted}));}});}else 
...
this.pending = current;};RelayContainer.prototype.
...
componentName):invariant(false):undefined;

return storeData.getQueuedStore().hasOptimisticUpdate(dataID);};RelayContainer.prototype.
...
return mutationIDs.map(RelayMutationTransaction.get);};RelayContainer.prototype.





getFragmentError = function getFragmentError(
fragmentReference,
record)
{
...
'conditions.'):invariant(false):undefined;

var fragmentID=fragment.getFragmentID();
var subscriptionKey=getSubscriptionKey(dataID,fragmentID);
return deferredErrors[subscriptionKey];};RelayContainer.prototype.
...
onFailure:this._handleDeferredFailure.bind(this)});}}else 
...
return hasData;};RelayContainer.prototype.
...
this.setState({
queryData:queryData,
variables:variables});};RelayContainer.prototype.
...
nullthrows(nextContext);var route=_nullthrows.route;
...
return {
variables:variables,
queryData:_this3._getQueryData(nextProps)};});};RelayContainer.prototype.
...
forEachObject(
this._queryResolvers,
function(queryResolver){return queryResolver && queryResolver.reset();});}
...
_updateQueryResolvers = function _updateQueryResolvers(){var _this4=this;
...
if(queryResolver){
queryResolver.reset();
queryResolvers[fragmentName] = null;}}else 
...
queryResolvers[fragmentName] = queryResolver;}});};RelayContainer.prototype.
...
if(fragmentPointer){
dataIDOrIDs = fragmentPointer.getDataID();}else 
{
...
fragmentName,
componentName,
propName):undefined;});});}};RelayContainer.prototype.
...
_handleDeferredSuccess = function _handleDeferredSuccess(
dataID,
fragmentID)
{
var subscriptionKey=getSubscriptionKey(dataID,fragmentID);
...
var queryData=this._getQueryData(this.props);
this.setState({queryData:queryData},deferredSuccessProfiler.stop);}};RelayContainer.prototype.
...
_handleDeferredFailure = function _handleDeferredFailure(
dataID,
fragmentID,
error)
{
...
this.setState(this.state,deferredFailureProfiler.stop);};RelayContainer.prototype.
...
ref:'component'})));};return RelayContainer;})(React.Component);
...
!
fragmentBuilder?process.env.NODE_ENV !== 'production'?invariant(false,
...
var fragments=spec.fragments;
!(
typeof fragments === 'object' && fragments)?process.env.NODE_ENV !== 'production'?invariant(false,
...
containerName,
fragmentName,
fragmentNames.map(function(name){return '`' + name + '`';}).join(', ')):invariant(false):undefined;}


!(
typeof fragmentBuilder === 'function')?process.env.NODE_ENV !== 'production'?invariant(false,
