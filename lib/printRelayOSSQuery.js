...
queryText = printField(node,printerState);}else 
if(node instanceof RelayQuery.Mutation){
queryText = printMutation(node,printerState);}

!
queryText?process.env.NODE_ENV !== 'production'?invariant(false,
'printRelayOSSQuery(): Unsupported node type.'):invariant(false):undefined;
...
{
!
!node.getBatchCall()?process.env.NODE_ENV !== 'production'?invariant(false,
'printRelayOSSQuery(): Deferred queries are not supported.'):invariant(false):undefined;
...
!
identifyingArgName?process.env.NODE_ENV !== 'production'?invariant(false,
...
!
inputString?process.env.NODE_ENV !== 'production'?invariant(false,
'printRelayOSSQuery(): Expected mutation `%s` to have a value for `%s`.',
node.getName(),
node.getCallVariableName()):invariant(false):undefined;
...
printerState)
{
var fragmentID=node.getFragmentID();var 
fragmentMap=printerState.fragmentMap;
...
'printRelayOSSQuery(): Query must be flattened before printing.'):invariant(false):undefined;
...
callsWithValues.forEach(function(_ref){var name=_ref.name;var value=_ref.value;
...
if(node instanceof RelayQuery.Field){
return printField(node,printerState);}else 
{
!(
...
function printDirective(_ref2){var name=_ref2.name;var value=_ref2.value;
!(
typeof value === 'boolean' || 
typeof value === 'number' || 
typeof value === 'string')?process.env.NODE_ENV !== 'production'?invariant(false,
'printRelayOSSQuery(): Relay only supports directives with scalar values ' + 
'(boolean, number, or string), got `%s: %s`.',
name,
value):invariant(false):undefined;
